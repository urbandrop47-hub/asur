// shop-pdp.jsx — product detail page (gallery · sizes · lore · specs · reviews)

const { useState: usePdpState } = React;

// build a placeholder "design" for a given gallery angle
function angleDesign(p, angleKey) {
  ...
  return p; // front = native stamp
}

function PdpGallery({ p }) {
  const [angle, setAngle] = usePdpState(0);
...
function SizeSelector({ book, value, onChange }) {
  const sold = book.soldSizes || [];
  return (
    <div className="pdp-sizes">
      ...
      <div className="pdp-size-row">
        {SIZE_RUN.map((s) => {
          const out = sold.includes(s);
          return (
            <button key={s}
                    className={`pdp-size ${value === s ? 'on' : ''} ${out ? 'out' : ''}`}
                    disabled={out}
                    onClick={() => !out && onChange(s)}>
              {s}
            </button>
          );
        })}
      </div>
      {sold.length > 0 && <div className="pdp-size-note">{sold.join(' · ')} sold out · no restock</div>}
    </div>
  );
}

function PdpLore({ p, book }) {
  return (
...
function ProductDetail({ p, onClose, onAdd, onOpen, cartCount, onCart }) {
  const [size, setSize] = usePdpState(null);
  const [added, setAdded] = usePdpState(false);
  ...
  return (
    <div className="pdp" data-screen-label={`PDP ${p.n}`}>
      ...
      <div className="pdp-scroll">
        ...
        <SizeSelector book={book} value={size} onChange={setSize} />
