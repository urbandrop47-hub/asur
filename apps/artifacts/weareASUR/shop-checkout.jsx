// shop-checkout.jsx — address → payment → confirmation
...
function Field({ label, value, onChange, placeholder, half, type = 'text' }) {
  return (
...
function Checkout({ items, onClose, onComplete }) {
  const [step, setStep] = useCoState(0);
  const [mode, setMode] = useCoState('saved');
  const [addr, setAddr] = useCoState({ name: '', phone: '', pin: '', line: '', city: '', state: '' });
  const [pay, setPay] = useCoState('upi');
  const [order, setOrder] = useCoState(null);
  ...
  return (
    <div className="co" data-screen-label={`Checkout · ${STEPS[step]}`}>
